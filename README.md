...
## ALSA/PipeWire Card Profile

The DSPi is a USB Audio Class device with a physical S/PDIF output. Without a
DSPi-specific ALSA card profile, PipeWire/PulseAudio can expose it as two
outputs and the digital output volume control may not work correctly.

The files in `alsa-card-profile/` install an ALSA card profile, mixer path, and
udev rule that make PipeWire/PulseAudio use a single `Digital Stereo (IEC958)`
profile for the DSPi and expose the USB Audio hardware volume control on the
digital output.
...
The installer copies the profile and path into the system ACP/PulseAudio mixer
directories, installs `/etc/udev/rules.d/91-dspi-pipewire-alsa.rules`, reloads
udev, retriggers the DSPi sound card if it is connected, and restarts the user
audio services when possible.
